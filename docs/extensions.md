...
ocx.policy(<a href="#ocx.policy-allow_unverified">allow_unverified</a>, <a href="#ocx.policy-allow_yanked">allow_yanked</a>, <a href="#ocx.policy-sigstore_trusted_root">sigstore_trusted_root</a>)
...
| <a id="ocx.project-bins"></a>bins |  Lazy provisioning: names of the executables to expose. When set, nothing is pulled at fetch time — each name becomes a launcher re-entering `ocx exec`, materializing the toolchain on first execution. Actions key on the lockfile, so fully remote-cached builds download no tool content.   | List of strings | optional |  `[]`  |
...
| <a id="ocx.project-ocx_lock"></a>ocx_lock |  The committed ocx.lock next to `ocx_toml` (watched; edits refetch).   | <a href="https://bazel.build/concepts/labels">Label</a> | required |  |
